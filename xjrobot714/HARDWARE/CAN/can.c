...
u8 CAN1_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
	...
	GPIO_Set(GPIOB,PIN8|PIN9,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_50M,GPIO_PUPD_PU);//PA11,PA12,复用功能,上拉输出
	...
	MY_NVIC_Init(0,3,CAN1_RX0_IRQn,2);//组2
#endif
	return 0;
}   
...
u32 Motor1EncodeCount=0;
u32 Motor2EncodeCount=0;
u32 Motor3EncodeCount=0;
u32 Motor4EncodeCount=0;
volatile u8 Motor1Error=0;
volatile u8 Motor2Error=0;
volatile u8 Motor3Error=0;
volatile u8 Motor4Error=0;

volatile float Motor1Current=0;
volatile float Motor2Current=0;
volatile float Motor3Current=0;
volatile float Motor4Current=0;
byte_to_float abb;
...
//    printf("rxbuf[7]:%d\r\n",rxbuf[7]);
		...
		if(id==0x581)   //ID为601时
		{
			if((rxbuf[0]==0x43)&&(rxbuf[1]==0x64)&&(rxbuf[2]==0x60)&&(rxbuf[3]==0x00))//里程计
			{
				Motor1EncodeCount=(rxbuf[7]<<24)+(rxbuf[6]<<16)+(rxbuf[5]<<8)+rxbuf[4];
			}
			if((rxbuf[0]==0x4B)&&(rxbuf[1]==0x1C)&&(rxbuf[2]==0x22)&&(rxbuf[3]==0x00))//电流,返回值为16，在4、5位
			{
				abb.bytes[0]=rxbuf[4];
				abb.bytes[1]=rxbuf[5];
				Motor1Current=abb.v;
			}
			if((rxbuf[0]==0x4F)&&(rxbuf[1]==0x01)&&(rxbuf[2]==0x10)&&(rxbuf[3]==0x00))//故障
			{
				Motor1Error=rxbuf[4];//只有4有值，56、位与速度有关
			}
		}
		if(id==0x582) 
		{
			if((rxbuf[0]==0x43)&&(rxbuf[1]==0x64)&&(rxbuf[2]==0x60)&&(rxbuf[3]==0x00))
			{
				Motor2EncodeCount=(rxbuf[7]<<24)+(rxbuf[6]<<16)+(rxbuf[5]<<8)+rxbuf[4];
			}
			if((rxbuf[0]==0x4B)&&(rxbuf[1]==0x1C)&&(rxbuf[2]==0x22)&&(rxbuf[3]==0x00))//电流
			{
				abb.bytes[0]=rxbuf[4];
				abb.bytes[1]=rxbuf[5];
				Motor2Current=abb.v;
		...
		if(id==0x583)
		{
			if((rxbuf[0]==0x43)&&(rxbuf[1]==0x64)&&(rxbuf[2]==0x60)&&(rxbuf[3]==0x00))
			{
				Motor3EncodeCount=(rxbuf[7]<<24)+(rxbuf[6]<<16)+(rxbuf[5]<<8)+rxbuf[4];
			}
			if((rxbuf[0]==0x4B)&&(rxbuf[1]==0x1C)&&(rxbuf[2]==0x22)&&(rxbuf[3]==0x00))//电流
			{
				abb.bytes[0]=rxbuf[4];
		...
		if(id==0x584)
		{
			if((rxbuf[0]==0x43)&&(rxbuf[1]==0x64)&&(rxbuf[2]==0x60)&&(rxbuf[3]==0x00))
			{
				Motor4EncodeCount=(rxbuf[7]<<24)+(rxbuf[6]<<16)+(rxbuf[5]<<8)+rxbuf[4];
			}
			if((rxbuf[0]==0x4B)&&(rxbuf[1]==0x1C)&&(rxbuf[2]==0x22)&&(rxbuf[3]==0x00))//电流
			{
				abb.bytes[0]=rxbuf[4];
